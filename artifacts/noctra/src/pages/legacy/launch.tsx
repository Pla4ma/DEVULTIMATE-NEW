...
export default function LaunchPage() {
  ...
  const goColor = goNoGo === "GO" ? "var(--color-success)" : goNoGo === "NO-GO" ? "var(--color-danger)" : "var(--color-warning)";
    ...
    <div className="space-y-4">
        ...
        >
          ...
          <div className="flex-1">
            ...
            <button
              ...
              style={{ background: "var(--color-danger)", color: "var(--surface-0)" }}
            >
      ...
      {projects.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-2">
            <FolderOpen size={13} style={{ color: "var(--text-tertiary)" }} />
            <label className="text-xs font-medium" style={{ color: "var(--text-tertiary)" }}>Load context from project</label>
          </div>
          <div className="flex gap-2 flex-wrap">
            {projects.map((p) => (
              <button
                ...
                style={{
                  background: selectedProjectId === p.id ? `${TOOL.accent}20` : "var(--surface-2)",
                  border: `1px solid ${selectedProjectId === p.id ? TOOL.accent : "var(--border-default)"}`,
                  color: selectedProjectId === p.id ? TOOL.accent : "var(--text-tertiary)",
                }}
              >
                {p.name}
              </button>
          ...
          {loadingContext && (
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className="text-xs" style={{ color: "var(--text-tertiary)" }}>Loading context…</span>
            </div>
          )}
      ...
      <div>
        ...
        {input.length > 0 && (
          ...
          {contextReports.length > 0 && (
            <button onClick={() => loadProjectContext(selectedProjectId).catch((err) => toast({ title: "Failed to reload context", description: err?.message, variant: "destructive" }))} className="flex items-center gap-1 mt-1.5 text-xs" style={{ color: "var(--text-tertiary)" }}>
              <RefreshCw size={10} /> Reload context into input
            </button>
          )}
          {projects.length === 0 && phase === "idle" && !input.trim() && (
            <div className="flex gap-2 mt-2">
              <button onClick={() => setInput("I have validated my idea, planned the MVP, and scanned my codebase. Ready for launch assessment.")} className="text-xs px-2.5 py-1 rounded-full" style={{ background: "var(--signal-soft)", border: "1px solid var(--accent-cyan-glow)", color: "var(--signal)" }}>
    ...
    phase === "idle" ? (
      ...
      <div className="flex items-center justify-center h-40">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm" style={{ color: "var(--text-tertiary)" }}>Running launch sequence…</p>
        </div>
      </div>
    ) : phase === "done" && result ? (
      <div className="space-y-4">
        ...
        {goNoGo && (
          <div className="flex items-center justify-between px-4 py-3 rounded-xl" style={{ background: `${goColor}10`, border: `1px solid ${goColor}30` }}>
            <div>
              ...
              {d?.verdict != null && <p className="text-xs mt-0.5" style={{ color: "var(--text-tertiary)" }}>{String(d.verdict)}</p>}
            </div>
            {d?.launch_score != null && (
        ...
        {saved && (
          <div className="flex gap-2 pt-1 border-t" style={{ borderColor: "var(--border-default)" }}>
            ...
            <NoctraButton variant="ghost" onClick={() => navigate(ROUTES.twin)} className="flex-1">
        ...
        {launchChecklist.length > 0 && (
          <div>
            <p className="eyebrow mb-2" style={{ color: "var(--text-tertiary)" }}>Launch Checklist</p>
              ...
              {launchChecklist.slice(0, 8).map((item, i) => {
                ...
                return (
                  <div key={i} className="flex items-start gap-2 text-xs">
                    <div className="mt-0.5 w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0" style={{ borderColor: done ? "var(--color-success)" : "var(--border-default)", background: done ? "var(--color-success-soft)" : "transparent" }}>
                      {done && <CheckCircle size={10} style={{ color: "var(--color-success)" }} />}
                    </div>
