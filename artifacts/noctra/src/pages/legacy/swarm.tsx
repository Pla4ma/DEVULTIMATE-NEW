...
export default function SwarmPage() {
  ...
  const InputPanel = (
    <div className="space-y-4">
      <div>
        <label className="block text-xs font-medium mb-2" style={{ color: "var(--text-tertiary)" }}>Your offer or pitch</label>
        <textarea
      ...
      <div>
        <label className="block text-xs font-medium mb-2" style={{ color: "var(--text-tertiary)" }}>Simulation depth</label>
        <div className="flex gap-2">
          {PERSONA_COUNTS.map((count) => (
            <button
              ...
              style={{
                background: personaCount === count ? `${TOOL.accent}20` : "var(--surface-2)",
                border: `1px solid ${personaCount === count ? TOOL.accent : "var(--border-default)"}`,
                color: personaCount === count ? TOOL.accent : "var(--text-tertiary)",
              }}
            >
              {count}
            </button>
      ...
      {/* Market segment */}
      <div>
        <label className="block text-xs font-medium mb-2" style={{ color: "var(--text-tertiary)" }}>Target Market Segment</label>
        <div className="flex flex-wrap gap-1.5">
          {MARKET_SEGMENTS.map((seg) => (
            <button
              ...
              style={{
                background: segment === seg ? `${TOOL.accent}20` : "var(--surface-2)",
                border: `1px solid ${segment === seg ? TOOL.accent : "var(--border-default)"}`,
                color: segment === seg ? TOOL.accent : "var(--text-tertiary)",
              }}
            >
              {seg}
            </button>
      ...
      {/* Price range */}
      <div>
        <label className="block text-xs font-medium mb-2" style={{ color: "var(--text-tertiary)" }}>Expected Price Range</label>
        <div className="flex flex-wrap gap-1.5">
          {PRICE_RANGES.map((pr) => (
            <button
              key={pr}
              ...
              className="px-2.5 py-1 rounded-full text-xs transition-all"
              style={{
                background: priceRange === pr ? `${TOOL.accent}20` : "var(--surface-2)",
                border: `1px solid ${priceRange === pr ? TOOL.accent : "var(--border-default)"}`,
                color: priceRange === pr ? TOOL.accent : "var(--text-tertiary)",
              }}
            >
              {pr}
            </button>
      ...
      <div>
        <label className="block text-xs font-medium mb-2" style={{ color: "var(--text-tertiary)" }}>
          Specific Personas to Include <span style={{ color: "var(--text-tertiary)", fontWeight: 400 }}>(optional)</span>
        </label>
        <input
    ...
    phase === "idle" ? (
      ...
      <div className="flex items-center justify-center h-40">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm" style={{ color: "var(--text-tertiary)" }}>Running {personaCount}-user market signal simulation…</p>
        </div>
      </div>
    ) : phase === "done" && result ? (
      <div className="space-y-4">
        ...
        {result.data && typeof result.data === "object" && "segment_breakdown" in result.data && (
          <div className="flex gap-4 px-4 py-3 rounded-xl" style={{ background: "var(--surface-2)", border: "1px solid var(--border-default)" }}>
            {Object.entries(result.data.segment_breakdown as Record<string, number>).map(([seg, pct]) => (
              <div key={seg} className="text-center flex-1">
                <p className="text-lg font-bold" style={{ color: seg === "enthusiasts" ? "var(--color-success)" : seg === "skeptics" ? "var(--color-danger)" : "var(--color-warning)" }}>{pct}%</p>
                <p className="text-xs capitalize" style={{ color: "var(--text-tertiary)" }}>{seg}</p>
              </div>
            ))}
          </div>
        )}
        ...
        {saved && (
          <div className="flex gap-2 pt-1 border-t" style={{ borderColor: "var(--border-default)" }}>
            ...
            <NoctraButton variant="ghost" onClick={() => navigate(ROUTES.mvp)} className="flex-1">
