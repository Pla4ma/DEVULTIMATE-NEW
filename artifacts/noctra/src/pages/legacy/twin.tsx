      ...
      <div className="p-6 max-w-6xl mx-auto space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-bold" style={{ color: "var(--text-primary)" }}>{TOOL.label}</h1>
            <p className="text-xs" style={{ color: "var(--text-tertiary)" }}>Ask about your project — patterns, blockers, and what to build next</p>
          </div>
        ...
        <Panel>
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs" style={{ color: "var(--text-tertiary)" }}>Project context:</span>
            </div>
            <div className="flex gap-2 flex-wrap">
              {[{ id: "all", name: "All" }, ...projects].map((p) => (
                <button key={p.id} onClick={() => setSelectedProjectId(p.id)} className="px-3 py-1 rounded-full text-xs transition-all" style={{ background: selectedProjectId === p.id ? `${TOOL.accent}20` : "var(--surface-2)", border: `1px solid ${selectedProjectId === p.id ? TOOL.accent : "var(--border-default)"}`, color: selectedProjectId === p.id ? TOOL.accent : "var(--text-tertiary)" }}>
                  {p.name}
                </button>
              ))}
            </div>
            {recentReports.length > 0 && (
              <span className="text-xs ml-auto shrink-0" style={{ color: "var(--text-tertiary)" }}>{recentReports.length} reports loaded</span>
            )}
          </div>
        </Panel>
          ...
          <Panel>
            ...
            <div className="space-y-2.5">
              {contradictions.map((c, i) => (
                <div key={i} className="px-3 py-2.5 rounded-lg" style={{ background: c.severity === "high" ? "var(--color-danger-soft)" : "var(--color-warning-soft)", border: `1px solid ${c.severity === "high" ? "var(--color-danger-soft)" : "var(--color-warning-soft)"}` }}>
                  <div className="flex items-start gap-2">
                    <Badge style={{ background: c.severity === "high" ? "var(--color-danger-soft)" : "var(--color-warning-soft)", color: c.severity === "high" ? "var(--color-danger)" : "var(--color-warning)", fontSize: "10px", flexShrink: 0 }}>{c.severity}</Badge>
                    <p className="text-xs" style={{ color: "var(--text-primary)" }}>{c.explanation}</p>
                  </div>
                  <div className="flex items-start gap-1.5 mt-2 ml-1">
                    <ChevronRight size={10} style={{ color: "var(--color-success)", marginTop: 1, flexShrink: 0 }} />
                    <p className="text-xs" style={{ color: "var(--text-tertiary)" }}>{c.recommendedResolution}</p>
                  </div>
                </div>
              ))}
            </div>
