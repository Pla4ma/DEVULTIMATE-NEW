      ...
      <div className="p-6 max-w-5xl mx-auto space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: "var(--text-tertiary)" }}>Measure signal density and generate your next experiments</p>
          </div>
        ...
        <div className="flex gap-1 p-1 rounded-xl" style={{ background: "var(--surface-1)" }}>
          {TABS.map(({ id, label }) => (
            <button key={id} onClick={() => setTab(id)} className="flex-1 px-4 py-2 rounded-lg text-sm font-medium transition-all" style={{ background: tab === id ? "var(--surface-2)" : "transparent", color: tab === id ? "var(--text-primary)" : "var(--text-tertiary)", border: tab === id ? "1px solid var(--border-default)" : "1px solid transparent" }}>
              {label}
            </button>
          ))}
        </div>
