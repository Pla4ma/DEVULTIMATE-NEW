...
export default function ReportDetailPage() {
  ...
  async function handleDelete() {
    if (!report) return;
    setDeleting(true);
    try { await deleteReport(report.id); navigate("/app/reports"); }
    ...
    switch (report.tool) {
      case "idea":
        return [
          ...
          { id: "gen-swarm", label: "Generate Market Swarm Report", description: "Simulate market demand with AI personas", icon: Zap, color: "var(--noctra-cyan)", busy: generatingSwarm },
      ...
      case "doctor":
        return [
          ...
          { id: "gen-launch-blockers", label: "Create Launch Blocker Tasks", description: "Generate tasks specifically for RED gates", icon: ClipboardList, color: "var(--noctra-rose)", busy: generatingTasks },
  ...
  async function handleToolAction(actionId: string) {
    ...
    switch (actionId) {
      case "gen-reality": {
        ...
        try { const saved = await generateChildReport("reality", buildContextFromReport(report)); if (saved?.id) { toast({ title: "Reality Report generated", description: "Navigating..." }); navigate(`/app/reports/${saved.id}`); } }
        finally { setGeneratingReality(false); }
        break;
      }
      case "gen-swarm": {
        setGeneratingSwarm(true);
        try { const saved = await generateChildReport("swarm", buildContextFromReport(report)); if (saved?.id) { toast({ title: "Market Swarm Report generated", description: "Navigating..." }); navigate(`/app/reports/${saved.id}`); } }
        finally { setGeneratingSwarm(false); }
        break;
      }
      case "gen-mvp": {
        setGeneratingMvp(true);
        try { const saved = await generateChildReport("mvp", buildContextFromReport(report)); if (saved?.id) { toast({ title: "MVP Report generated", description: "Navigating..." }); navigate(`/app/reports/${saved.id}`); } }
        finally { setGeneratingMvp(false); }
        break;
      }
      case "gen-proof": {
        setGeneratingProof(true);
        try { const saved = await generateChildReport("proof", buildContextFromReport(report)); if (saved?.id) { toast({ title: "Proof Report generated", description: "Navigating..." }); navigate(`/app/reports/${saved.id}`); } }
        finally { setGeneratingProof(false); }
        break;
      }
      case "gen-launch": {
        setGeneratingLaunch(true);
        try { const saved = await generateChildReport("launch", buildContextFromReport(report)); if (saved?.id) { toast({ title: "Launch Report generated", description: "Navigating..." }); navigate(`/app/reports/${saved.id}`); } }
      ...
      case "gen-sprint": {
        ...
        try {
          ...
          const sid = (saved as unknown as { id?: string })?.id;
          if (sid) { toast({ title: "Sprint generated and saved!", description: "Navigating to sprint report." }); navigate(`/app/reports/${sid}`); }
          else { const md = sprintToMarkdown(sprint as unknown as Record<string, unknown>); downloadMarkdown(`${report.title.replace(/\s+/g, "-").toLowerCase()}-sprint`, md); toast({ title: "Sprint downloaded", description: "Could not save as report — downloaded as markdown." }); }
      ...
      case "gen-prompt-pack": {
        ...
        try {
          ...
          const sid = (saved as unknown as { id?: string })?.id;
          if (sid) { toast({ title: "Prompt Pack saved!", description: "Navigating to prompt pack report." }); navigate(`/app/reports/${sid}`); }
      ...
      case "copy-doctor-prompt": {
        ...
        const prompt = generateDevAgentPrompt({
          ...
          state: { phase: "launch-prep", readiness: (dpData.launch_readiness_score as number) ?? (report.score ?? 0), doctorScore: report.score ?? 0, failedGates: Array.isArray(dpData.red_gates) ? (dpData.red_gates as string[]) : [], topBlocker: (dpData.top_blocker as string) ?? null, nextAction: { title: (dpData.recommended_action as string) ?? "Fix launch blockers", href: "/app/tasks", reason: "", description: "", priority: "high", tool: "doctor" }, ideaScore: 0, realityScore: 0, proofScore: 0, swarmScore: 0, mvpScore: 0, launchScore: 0, overallScore: 0, coveredTools: [], missingTools: [], openP0Tasks: 0, openP1Tasks: 0, latestReportByTool: {}, proofSignalCount: 0, scanCount: 0, totalReports: 0, totalTasks: 0, completedTasks: 0, taskCompletionRate: 0 },
  ...
  if (loading) {
    return (<AppShell><div className="flex items-center justify-center h-64"><Loader2 size={24} className="animate-spin" style={{ color: "var(--noctra-cyan)" }} /></div></AppShell>);
  }
    ...
    return (
      <AppShell>
        <div className="p-6 max-w-4xl mx-auto">
          ...
          <div className="flex justify-center mt-4"><NoctraButton variant="ghost" onClick={() => navigate("/app/reports")}><ArrowLeft size={14} /> Back to Reports</NoctraButton></div>
        </div>
      </AppShell>
    );
  }
  ...
  function renderActionButton(action: ToolAction) {
    return (
      <button key={action.id} onClick={() => handleToolAction(action.id)} disabled={action.busy} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-all hover:opacity-80 disabled:opacity-50" style={{ background: "var(--noctra-surface2)", border: "1px solid var(--noctra-border)" }}>
  ...
  function renderNextActionButton(na: typeof nextActions[number]) {
    ...
    return (
      <button key={na.href + na.title} onClick={() => navigate(na.href)} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-all hover:opacity-90" style={{ background: "var(--noctra-surface2)", border: "1px solid var(--noctra-border)" }}>
    ...
    <AppShell>
      ...
      <div className="p-6 max-w-5xl mx-auto space-y-5">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <button onClick={() => navigate("/app/reports")} className="flex items-center gap-1.5 text-sm hover:opacity-80" style={{ color: "var(--noctra-text-muted)" }}><ArrowLeft size={14} /> All Reports</button>
          <div className="flex items-center gap-2 flex-wrap">
            {confirmDelete ? (
              ...
              <NoctraButton variant="ghost" onClick={() => setConfirmDelete(true)}><Trash2 size={13} style={{ color: "var(--noctra-rose)" }} /><span style={{ color: "var(--noctra-rose)" }}>Delete</span></NoctraButton>
            )}
          </div>
        </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-2 text-xs flex-wrap" style={{ color: "var(--noctra-text-muted)" }}>
                ...
                {linkedProject ? <button onClick={() => navigate(`/app/projects/${report.project_id}`)} className="flex items-center gap-1 hover:opacity-80" style={{ color: "var(--noctra-cyan)" }}><FolderOpen size={11} /> {linkedProject.name}</button> : null}
              </div>
            </div>
          </div>
        </Panel>
        ...
        {monetization ? (
          <Panel>
            ...
            <div className="space-y-3 text-xs" style={{ color: "var(--noctra-text)" }}>
              <div className="flex gap-4 flex-wrap"><div><span className="font-semibold" style={{ color: "var(--noctra-text-muted)" }}>Model:</span> {monetization.bestModel}</div><div><span className="font-semibold" style={{ color: "var(--noctra-text-muted)" }}>Strategy:</span> {monetization.paywallStrategy.slice(0, 80)}…</div></div>
        ...
        {continueWorkflowActions.length + workflowNextActions.length > 0 ? (
          <Panel>
            <div className="flex items-center gap-2 mb-3"><Rocket size={12} style={{ color: accentColor }} /><p className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--noctra-text-muted)" }}>Continue Workflow</p></div>
        ...
        {createExecutionActions.length + executionNextActions.length > 0 ? (
          <Panel>
            <div className="flex items-center gap-2 mb-3"><Zap size={12} style={{ color: accentColor }} /><p className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--noctra-text-muted)" }}>Create Execution</p></div>
        ...
        <Panel>
          <div className="flex items-center gap-2 mb-3"><Download size={12} style={{ color: "var(--noctra-text-muted)" }} /><p className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--noctra-text-muted)" }}>Export</p></div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <button onClick={handleCopySummary} className="flex flex-col items-center gap-1.5 px-3 py-3 rounded-xl text-xs font-medium hover:opacity-80" style={{ background: summaryCopied ? "rgba(52,211,153,0.1)" : "var(--noctra-surface2)", border: `1px solid ${summaryCopied ? "var(--noctra-emerald)" : "var(--noctra-border)"}`, color: summaryCopied ? "var(--noctra-emerald)" : "var(--noctra-text)" }}>
              {summaryCopied ? <CheckCheck size={15} /> : <Copy size={15} />}{summaryCopied ? "Copied!" : "Copy Summary"}
            </button>
            <button onClick={handleDownloadMD} className="flex flex-col items-center gap-1.5 px-3 py-3 rounded-xl text-xs font-medium hover:opacity-80" style={{ background: "var(--noctra-surface2)", border: "1px solid var(--noctra-border)", color: "var(--noctra-text)" }}><Download size={15} /> Download MD</button>
            <button onClick={handleDownloadJSON} className="flex flex-col items-center gap-1.5 px-3 py-3 rounded-xl text-xs font-medium hover:opacity-80" style={{ background: "var(--noctra-surface2)", border: "1px solid var(--noctra-border)", color: "var(--noctra-text)" }}><FileText size={15} /> Download JSON</button>
            {exportShareActions.map((action) => (
              <button key={action.id} onClick={() => handleToolAction(action.id)} disabled={action.busy} className="flex flex-col items-center gap-1.5 px-3 py-3 rounded-xl text-xs font-medium hover:opacity-80 disabled:opacity-50" style={{ background: "var(--noctra-surface2)", border: "1px solid var(--noctra-border)", color: "var(--noctra-text)" }}>
        ...
        <Panel>
          <div className="flex items-center gap-2 mb-3"><Link2 size={12} style={{ color: "var(--noctra-text-muted)" }} /><p className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--noctra-text-muted)" }}>Manage</p></div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <button onClick={() => setShowLinkModal(true)} className="flex flex-col items-center gap-1.5 px-3 py-3 rounded-xl text-xs font-medium hover:opacity-80" style={{ background: linkedProject ? "rgba(61,216,255,0.08)" : "var(--noctra-surface2)", border: `1px solid ${linkedProject ? "rgba(61,216,255,0.2)" : "var(--noctra-border)"}`, color: linkedProject ? "var(--noctra-cyan)" : "var(--noctra-text)" }}><Link2 size={15} />{linkedProject ? "Relink" : "Link Project"}</button>
            {!report.project_id ? (
              <button onClick={handleCreateProject} disabled={creatingProject} className="flex flex-col items-center gap-1.5 px-3 py-3 rounded-xl text-xs font-medium hover:opacity-80 disabled:opacity-50" style={{ background: "rgba(52,211,153,0.08)", border: "1px solid rgba(52,211,153,0.2)", color: "var(--noctra-emerald)" }}>
                {creatingProject ? <Loader2 size={15} className="animate-spin" /> : <Plus size={15} />}{creatingProject ? "Creating…" : "New Project"}
              </button>
            ) : (
              <button onClick={() => navigate(`/app/projects/${report.project_id}`)} className="flex flex-col items-center gap-1.5 px-3 py-3 rounded-xl text-xs font-medium hover:opacity-80" style={{ background: "var(--noctra-surface2)", border: "1px solid var(--noctra-border)", color: "var(--noctra-text)" }}><FolderOpen size={15} /> View Project</button>
            )}
            <button onClick={() => setConfirmDelete(true)} className="flex flex-col items-center gap-1.5 px-3 py-3 rounded-xl text-xs font-medium hover:opacity-80" style={{ background: "rgba(244,63,94,0.08)", border: "1px solid rgba(244,63,94,0.2)", color: "var(--noctra-rose)" }}><Trash2 size={15} /> Delete</button>
