      ...
      {addingSignal ? (
        <Panel>
          ...
          <div className="space-y-2">
            <input value={signalLabel} onChange={(e) => onSignalLabelChange(e.target.value)} placeholder="What did you validate?" className="w-full px-3 py-2 rounded-lg text-sm outline-none" style={{ background: "var(--noctra-surface2)", border: "1px solid var(--noctra-border)", color: "var(--noctra-text)" }} />
            <div className="flex gap-2">
              {["qualitative", "quantitative", "behavioral"].map((k) => (
                <button key={k} onClick={() => onSignalKindChange(k)} className="px-3 py-1 rounded-full text-xs capitalize" style={{ background: signalKind === k ? "rgba(52,211,153,0.15)" : "var(--noctra-surface2)", border: `1px solid ${signalKind === k ? "var(--noctra-emerald)" : "var(--noctra-border)"}`, color: signalKind === k ? "var(--noctra-emerald)" : "var(--noctra-text-muted)" }}>{k}</button>
              ))}
            </div>
            <input value={signalSource} onChange={(e) => onSignalSourceChange(e.target.value)} placeholder="Source (optional)" className="w-full px-3 py-2 rounded-lg text-sm outline-none" style={{ background: "var(--noctra-surface2)", border: "1px solid var(--noctra-border)", color: "var(--noctra-text)" }} />
            <textarea value={signalEvidence} onChange={(e) => onSignalEvidenceChange(e.target.value)} placeholder="Evidence / notes (optional)" rows={2} className="w-full px-3 py-2 rounded-lg text-sm resize-none outline-none" style={{ background: "var(--noctra-surface2)", border: "1px solid var(--noctra-border)", color: "var(--noctra-text)" }} />
            <div className="flex gap-2">
