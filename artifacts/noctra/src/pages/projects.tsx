    ...
    <AppShell>
      <div className="p-6 max-w-4xl mx-auto space-y-5">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {STAGES.map((s) => (
                    <button key={s} onClick={() => setStage(s)} className="px-3 py-1 rounded-full text-xs font-medium capitalize transition-all" style={{ background: stage === s ? `${STAGE_COLORS[s]}20` : "var(--noctra-surface2)", border: `1px solid ${stage === s ? STAGE_COLORS[s] : "var(--noctra-border)"}`, color: stage === s ? STAGE_COLORS[s] : "var(--noctra-text-muted)" }}>
                      {s}
                    </button>
                  ))}
                </div>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {filtered.map((proj) => {
              const stageColor = STAGE_COLORS[proj.stage ?? "idea"] ?? "var(--noctra-text-muted)";
