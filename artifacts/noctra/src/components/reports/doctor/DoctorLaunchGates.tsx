  ...
  return (
    <Panel>
      <div className="flex items-center gap-2 mb-3">
        ...
        <p className="eyebrow" style={{ color: "var(--text-tertiary)" }}>Launch Gates</p>
      ...
      <div className="space-y-2">
        {gates.map((g, i) => (
          <div key={i} className="rounded-xl p-3" style={{
            background: g.status === "RED" ? "var(--color-danger-soft)" : g.status === "YELLOW" ? "var(--color-warning-soft)" : "var(--color-success-soft)",
            ...
            {g.evidence && g.evidence.length > 0 && (
              <ul className="mt-1 space-y-0.5">
                {g.evidence.map((e, j) => (
                  <li key={j} className="text-xs flex items-start gap-1.5" style={{ color: "var(--text-tertiary)" }}>
