            ...
            <div className="p-3" onClick={() => setMobileOpen(false)}>
              <div className="flex flex-col h-full">
                  ...
                  {NAV_ITEMS.map((item) => {
                    ...
                    return (
                      <Link key={item.href} href={item.href}>
                        <div
                          className={cn(
                            "flex items-center gap-3 px-3 py-2.5 rounded-lg cursor-pointer transition-all duration-300",
                            isActive ? "text-accent" : "hover:bg-white/5"
                          )}
                          style={{
                            background: isActive
                              ? "linear-gradient(135deg, rgba(168,85,247,0.1) 0%, rgba(232,121,249,0.05) 100%)"
                ...
                <div className="border-t pt-2 space-y-1" style={{ borderColor: "rgba(255,255,255,0.04)" }}>
                  <button className="flex items-center gap-3 w-full px-3 py-2.5 rounded-lg transition-all duration-300 hover:bg-white/5" style={{ color: "#6b6580" }}>
                    <Settings size={18} strokeWidth={1.5} />
                    <span className="text-sm">Settings</span>
                  </button>
                  <button
                    onClick={() => signOut()}
                    className="flex items-center gap-3 w-full px-3 py-2.5 rounded-lg transition-all duration-300 hover:bg-red-500/10"
                    style={{ color: "#6b6580" }}
                  >
